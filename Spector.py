...
def CleanCalibs():
    os.mkdir('calibs')
    templist=g.glob('f_s_r*')
    templist2=g.glob('b_s_r*')
    for i in range(0,len(templist)):
...
def TrimFiles():
    ...
    templist=g.glob('i_s_r*')
    templist2=g.glob('a_s_r*')
    for i in range(0,len(templist)):
